...
* Every device that clears the threshold gets its own exclusive tab — Controls/Settings/Diagnostics/PTZ pills for that device render together inside it, each under its own small caption (bundling multiple role-pills into one device tab loses the old per-tab label as the boundary between them; the caption restores it without forcing a color on Diagnostics, which stays neutral on purpose — see the weather-chip section for the styling philosophy). A device that doesn't clear the threshold (≤1 qualifying entity, or no `device_id` at all) folds into a shared "Other" tab instead of getting a dedicated one. Tab order: the camera's device first (if any), then by descending entity count, "Other" last. Tab label resolves from the device registry's own name, falling back to the entities' common `entity_id` prefix. A tab's content has no scroll or height cap — a busy device (a real IR blaster can expose 25+ segments) grows the card instead of hiding entities behind a nested scroll region, which is a poor fit for a dashboard tile that's already inside a scrolling page.
