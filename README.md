...
[![Version](https://img.shields.io/badge/version-2.6.1-green.svg)](src/constants.js)
...
| History chart | Background sparkline, points spaced by real elapsed time; min/max/period/trend labels; fill zones colored by threshold (clipPath, not gradient); hover/tap a point for its exact value; "No numeric history" note instead of a silent blank area for the wrong entity domain |
...
# ── Entity chips ──────────────────────────────────────────────────────
show_entities: true        # Show the entity chip strip (default: true) — also gates the controls/settings rows
max_entities: 12           # Max chips to display (default: 12)
...
The sparkline overlays small labels on the card: `↑ max` (top-right), `↓ min` (bottom-right), and time window (bottom-left, hover/tap it to see which entity is charted — it can differ from the averaged value shown in the header). A trend badge (top-left) compares the second half of the visible window against the first half: `⬈` (orange) when it's meaningfully higher, `⬊` (blue) when meaningfully lower, `➡` (grey) when the swing is under 5% of the window's own range, and no badge at all with fewer than 4 points to compare. These four corner badges render slightly dimmed (55% opacity) so they read as chrome, not competing with the line itself. When `threshold_high` or `threshold_low` are set, the threshold value (e.g. `22.0°C`) is also printed directly on the dashed marker line, left-aligned and centered on the line, nudged inward if it would otherwise land under one of the corner labels. Threshold zones use SVG `clipPath` — only the portion of the fill that actually exceeds/dips below a threshold is colored, so zone coloring tracks the real data.

Each history point keeps its real timestamp, so points are spaced by actual elapsed time, not array order — a sensor that reports unevenly (bursts of readings, then a quiet overnight gap) still draws a trend shape that matches reality. Hovering (desktop) or tapping (touch) a point shows its exact value in a small pill; dense series (many points close together) also show a round marker dot at that point, positioned independently of the chart's own stretch so it always renders round.

When all history points are identical (e.g. a brand-new sensor with only one reading) and no `y_min`/`y_max` is set, the sparkline is hidden — there is no meaningful shape to draw. Set `y_min: 0` to force the chart to render with the value positioned relative to zero. If `history_chart.entity_id` resolves to fewer than 2 numeric readings (wrong domain, e.g. a `binary_sensor`, or no history yet), the card shows a small "No numeric history" note instead of a silently blank area.

The chart is fetched once via `hass.callWS` and cached for 5 minutes (and pruned once stale, so a dashboard left open for days doesn't leak cache entries). First render shows no chart; the SVG appears after the async response resolves — typically imperceptible on a normal page load. When `add_entities` or `history_chart.entity_id` entities are used, their state changes also trigger re-renders and TTL refresh.
