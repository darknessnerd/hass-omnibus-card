...
`;function ue(t,e,a){const n=(t==null?void 0:t.y_min)!=null?Math.min(t.y_min,e):e,r=(t==null?void 0:t.y_max)!=null?Math.max(t.y_max,a):a;return{min:n,max:r,range:r-n}}function Ke(t,e=150){if(t.length<=e)return t.slice();const a=Math.floor(e/2),n=t.length/a,r=[];for(let i=0;i<a;i++){const s=Math.floor(i*n),c=i===a-1?t.length:Math.floor((i+1)*n);if(s>=c)continue;let o=-1,d=-1;for(let h=s;h<c;h++)Number.isFinite(t[h].v)&&((o===-1||t[h].v<t[o].v)&&(o=h),(d===-1||t[h].v>t[d].v)&&(d=h));if(o===-1)r.push(t[s]);else if(o===d)r.push(t[o]);else{const[h,l]=o<d?[o,d]:[d,o];r.push(t[h],t[l])}}return r}const Je=40,J=14,W=new WeakMap;function Qe(t,e,a=null,n=""){if(!(t!=null&&t.length)||t.length<2)return"";const r=W.get(t);if(r&&r.color===e&&r.hc===a&&r.unit===n)return r.result;const i=300,s=60,c=t.map(m=>m.v),o=Math.min(...c),d=Math.max(...c),{min:h,range:l}=ue(a,o,d);if(l===0&&(a==null?void 0:a.y_min)==null&&(a==null?void 0:a.y_max)==null)return W.set(t,{color:e,hc:a,unit:n,result:""}),"";const p=l||1,u=Ke(t),f=t[0].t,k=t[t.length-1].t-f||1,E=u.map(m=>(m.t-f)/k*i),S=u.map(m=>s-(m.v-h)/p*s),j=`${E.map((m,w)=>`${w?"L":"M"}${m.toFixed(1)},${S[w].toFixed(1)}`).join(" ")} V${s} H0 Z`,x=u.length>Je,b=x?"":E.map((m,w)=>`<circle cx="${m.toFixed(1)}" cy="${S[w].toFixed(1)}" r="1.5" fill="${e}"/>`).join(""),Q=i/(E.length-1),F=Math.min(4,Q/2).toFixed(1),A=E.map((m,w)=>{if(!Number.isFinite(u[w].v))return"";const X=`${u[w].v.toFixed(1)}${n}`;return`<circle cx="${m.toFixed(1)}" cy="${S[w].toFixed(1)}" r="${F}" fill="transparent" data-v="${X}"/>`}).join(""),U=`<svg class="chart-hit-layer${x?" dense":""}" viewBox="0 0 ${i} ${s}" preserveAspectRatio="none" aria-hidden="true">${A}</svg>`;if(!(a&&(a.threshold_high!=null||a.threshold_low!=null))){const m=ge(i,s,`<path d="${j}" fill="${e}"/>${b}`)+U;return W.set(t,{color:e,hc:a,unit:n,result:m}),m}const ee=a.color??"rgba(3, 169, 244, 0.12)",Y=a.color_high??"rgba(244, 67, 54, 0.25)",P=a.color_low??"rgba(33, 150, 243, 0.25)",D=m=>Math.max(0,Math.min(s,s-(m-h)/p*s)),q=s*(J/100),Z=m=>Math.min(s-q,Math.max(q,m)),V=`<defs><clipPath id="sg-cp"><path d="${j}"/></clipPath></defs>`;let T=`<path d="${j}" fill="${ee}"/>`;if(a.threshold_high!=null){const m=D(a.threshold_high);if(m>0&&(T+=`<rect x="0" y="0" width="${i}" height="${m.toFixed(1)}" fill="${Y}" clip-path="url(#sg-cp)"/>`),m>0&&m<s){const w=Z(m).toFixed(1);T+=`<line x1="0" y1="${w}" x2="${i}" y2="${w}" stroke="${Y}" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.6"/>`}}if(a.threshold_low!=null){const m=D(a.threshold_low);if(m<s&&(T+=`<rect x="0" y="${m.toFixed(1)}" width="${i}" height="${(s-m).toFixed(1)}" fill="${P}" clip-path="url(#sg-cp)"/>`),m>0&&m<s){const w=Z(m).toFixed(1);T+=`<line x1="0" y1="${w}" x2="${i}" y2="${w}" stroke="${P}" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.6"/>`}}const B=ge(i,s,V+T+b)+U;return W.set(t,{color:e,hc:a,unit:n,result:B}),B}function ge(t,e,a){return`<svg class="bg-chart" viewBox="0 0 ${t} ${e}" preserveAspectRatio="none" aria-hidden="true">${a}</svg>`}function et({areaName:t,cardIcon:e,hasLights:a,lightCount:n,offlineLights:r,occupied:i,hasOccupancySensors:s,problemCount:c,showBatteryBadge:o,batteryValue:d,batteryIcon:h,batteryEntity:l,batteryTitle:p,updateCount:u,updateEntity:f,updateTitle:_,openingItems:k,tamperItems:E}){const S=n===0,y=S?r>0?`${r} light${r!==1?"s":""} offline`:"Lights off":`${n} light${n!==1?"s":""} on${r>0?` · ${r} offline`:""}`;return`
    <div class="header">
      ...
      <div class="header-right">
        ...
        ${tt({showBatteryBadge:o,batteryValue:d,batteryIcon:h,batteryEntity:l,batteryTitle:p,problemCount:c,updateCount:u,updateEntity:f,updateTitle:_})}
      </div>
    </div>`}function tt({showBatteryBadge:t,batteryValue:e,batteryIcon:a,batteryEntity:n,batteryTitle:r,problemCount:i,updateCount:s,updateEntity:c,updateTitle:o}){const d=[];return t&&d.push(`
    ...
    </span>`),d.length?`<div class="chip group-chip status-cluster" title="Alerts">${d.join("")}</div>`:""}function at({tempVal:t,humVal:e,tempUnit:a,tempEntities:n,humEntities:r,climate:i,climIcon:s,climColor:c}){var l,p,u,f,_,k,E,S;if(t===null&&e===null&&!s)return"";const o=n.length>1?`Avg of ${n.length} sensors`:((p=(l=n[0])==null?void 0:l.state.attributes)==null?void 0:p.friendly_name)??"",d=r.length>1?`Avg of ${r.length} sensors`:((f=(u=r[0])==null?void 0:u.state.attributes)==null?void 0:f.friendly_name)??"",h=((_=i==null?void 0:i.state.attributes)==null?void 0:_.friendly_name)??(i==null?void 0:i.entityId)??"";return`
    <div class="env-row">
      ${t!==null?`
    ...
    ${n}
    `}function ct({hasCamera:t,cameraImage:e,cameraIcon:a,cameraEntity:n,cameraTitle:r,cameraState:i,cameraOffline:s,cameraPrivacy:c}){if(!t)return"";const o=c?`${r} (privacy mode)`:s?`${r} (offline)`:r,d=e&&!c;return`
    <div class="camera-preview${s?" offline":""}${c?" privacy":""}" data-entity="${n}"
         role="button" tabindex="0" aria-label="${o}" title="${o}">
      ${d?`<img src="${e}" alt="${o}" loading="lazy" />`:`<div class="camera-placeholder"><ha-icon icon="${c?"mdi:eye-off":a}"></ha-icon></div>`}
      ${i==="recording"&&!c?'<span class="camera-rec-dot" title="Recording"></span>':""}
      ${d?`
    ...
    <div class="device-role">
      ...
      ${a}
    </div>`).join("")}function ft({deviceGroups:t,collapsibleControls:e,activeSection:a}){const n=t.map(({key:r,label:i,icon:s,ptz:c,controls:o,settings:d,diagnostics:h})=>({key:r,label:i,icon:s,roleSections:gt({ptz:c,controls:o,settings:d,diagnostics:h})})).filter(r=>r.roleSections.length);return n.length?e?`
    ...
    </ha-card>`}function vt({historyMin:t,historyMax:e,historyUnit:a,historyHours:n,historyChart:r,historyEmpty:i}){if(t===null)return i?'<div class="chart-overlay"><span class="chart-stat chart-empty">No numeric history</span></div>':"";const s=[];if((r==null?void 0:r.threshold_high)!=null||(r==null?void 0:r.threshold_low)!=null){const{min:c,range:o}=ue(r,t,e),d=o||1,h=p=>(1-(p-c)/d)*100,l=p=>Math.min(100-J,Math.max(J,p));if(r.threshold_high!=null){const p=h(r.threshold_high);p>0&&p<100&&s.push(`<span class="chart-threshold" style="top:${l(p).toFixed(1)}%">${r.threshold_high.toFixed(1)}${a}</span>`)}if(r.threshold_low!=null){const p=h(r.threshold_low);p>0&&p<100&&s.push(`<span class="chart-threshold" style="top:${l(p).toFixed(1)}%">${r.threshold_low.toFixed(1)}${a}</span>`)}}return`
