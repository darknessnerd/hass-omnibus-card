...
`;function X(t){const e=t.map(a=>parseFloat(a.state.state)).filter(a=>!isNaN(a));return e.length?e.reduce((a,n)=>a+n,0)/e.length:null}function z(t){return t.some(e=>e.state.state==="on")}function ye(t){return t.filter(e=>e.state.state==="on")}function ve(t){let e=null;for(const a of t){const n=parseFloat(a.state.state);isNaN(n)||(!e||n<e.value)&&(e={value:n,entityId:a.entityId,state:a.state})}return e}function xe(t){var e;for(const a of t){const n=(e=a.state.attributes)==null?void 0:e.rgb_color;if(n)return`rgb(${n.join(",")})`}return null}const _e=/_(max|gust|peak)$/i;function J(t,e){var n;return(((n=e.attributes)==null?void 0:n.friendly_name)??t.split(".")[1]).split(" ").pop()}function T(t,e){var l,s;if((l=e.attributes)!=null&&l.icon)return e.attributes.icon;const a=t.split(".")[0],n=((s=e.attributes)==null?void 0:s.device_class)??"",i=q.has(e.state),r=o=>typeof o=="string"?o:i?o.on:o.off;return a==="sensor"&&n==="battery"?K(parseFloat(e.state)):n==="wind_speed"&&_e.test(t)?pe:n&&U[n]?r(U[n]):Y[a]?r(Y[a]):"mdi:help-circle-outline"}function K(t){if(t==null||isNaN(t))return"mdi:battery-unknown";const e=Math.min(100,Math.max(0,t));return e<=5?"mdi:battery-alert-variant-outline":e>=100?"mdi:battery":`mdi:battery-${Math.min(90,Math.max(10,Math.round(e/10)*10))}`}function S(t,e){t.dispatchEvent(new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:e}}))}function $e(t){history.pushState(null,"",t),window.dispatchEvent(new CustomEvent("location-changed",{bubbles:!0,composed:!0,detail:{replace:!1}}))}function Q(t,e,a){const n=(t==null?void 0:t.y_min)!=null?Math.min(t.y_min,e):e,i=(t==null?void 0:t.y_max)!=null?Math.max(t.y_max,a):a;return{min:n,max:i,range:i-n}}function we(t,e=150){if(t.length<=e)return t.map((r,l)=>({value:r,index:l}));const a=Math.floor(e/2),n=t.length/a,i=[];for(let r=0;r<a;r++){const l=Math.floor(r*n),s=r===a-1?t.length:Math.floor((r+1)*n);if(l>=s)continue;let o=-1,p=-1;for(let d=l;d<s;d++)Number.isFinite(t[d])&&((o===-1||t[d]<t[o])&&(o=d),(p===-1||t[d]>t[p])&&(p=d));if(o===-1)i.push({value:t[l],index:l});else if(o===p)i.push({value:t[o],index:o});else{const[d,c]=o<p?[o,p]:[p,o];i.push({value:t[d],index:d},{value:t[c],index:c})}}return i}const ke=40;function Se(t,e,a=null,n=""){if(!(t!=null&&t.length)||t.length<2)return"";const i=300,r=60,l=Math.min(...t),s=Math.max(...t),{min:o,range:p}=Q(a,l,s);if(p===0&&(a==null?void 0:a.y_min)==null&&(a==null?void 0:a.y_max)==null)return"";const d=p||1,c=we(t),u=t.length,h=c.map(g=>g.index/(u-1)*i),v=c.map(g=>r-(g.value-o)/d*r),_=`${h.map((g,$)=>`${$?"L":"M"}${g.toFixed(1)},${v[$].toFixed(1)}`).join(" ")} V${r} H0 Z`,I=c.length>ke,y=I?"":h.map((g,$)=>`<circle cx="${g.toFixed(1)}" cy="${v[$].toFixed(1)}" r="1.5" fill="${e}"/>`).join(""),B=i/(h.length-1),x=Math.min(4,B/2).toFixed(1),f=h.map((g,$)=>Number.isFinite(c[$].value)?`<circle cx="${g.toFixed(1)}" cy="${v[$].toFixed(1)}" r="${x}" fill="transparent"><title>${c[$].value.toFixed(1)}${n}</title></circle>`:"").join(""),P=`<svg class="chart-hit-layer${I?" dense":""}" viewBox="0 0 ${i} ${r}" preserveAspectRatio="none" aria-hidden="true">${f}</svg>`;if(!(a&&(a.threshold_high!=null||a.threshold_low!=null)))return R(i,r,`<path d="${_}" fill="${e}"/>${y}`)+P;const j=a.color??"rgba(3, 169, 244, 0.12)",M=a.color_high??"rgba(244, 67, 54, 0.25)",F=a.color_low??"rgba(33, 150, 243, 0.25)",N=g=>Math.max(0,Math.min(r,r-(g-o)/d*r)),D=`<defs><clipPath id="sg-cp"><path d="${_}"/></clipPath></defs>`;let C=`<path d="${_}" fill="${j}"/>`;if(a.threshold_high!=null){const g=N(a.threshold_high);g>0&&(C+=`<rect x="0" y="0" width="${i}" height="${g.toFixed(1)}" fill="${M}" clip-path="url(#sg-cp)"/>`),g>0&&g<r&&(C+=`<line x1="0" y1="${g.toFixed(1)}" x2="${i}" y2="${g.toFixed(1)}" stroke="${M}" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.6"/>`)}if(a.threshold_low!=null){const g=N(a.threshold_low);g<r&&(C+=`<rect x="0" y="${g.toFixed(1)}" width="${i}" height="${(r-g).toFixed(1)}" fill="${F}" clip-path="url(#sg-cp)"/>`),g>0&&g<r&&(C+=`<line x1="0" y1="${g.toFixed(1)}" x2="${i}" y2="${g.toFixed(1)}" stroke="${F}" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.6"/>`)}return R(i,r,D+C+y)+P}function R(t,e,a){return`<svg class="bg-chart" viewBox="0 0 ${t} ${e}" preserveAspectRatio="none" aria-hidden="true">${a}</svg>`}function Ce(t,e,a=null,n=!1){var j,M,F,N,D,C,g,$,ee,te,ae,ie,re,oe;const i=e.area,r=(j=t.areas)==null?void 0:j[i];if(!r&&!e.name&&!((M=e.entities)!=null&&M.length))return{error:i??"(no area)"};const l=(F=e.entities)!=null&&F.length?[]:Z(t,i),s=ue(l,e,t),o=be(s),p=ye(o.lights),d=xe(p),c=X(o.temperatures),u=X(o.humidities),h=o.climate[0]??null,[v,k]=de[(N=h==null?void 0:h.state)==null?void 0:N.state]??[null,null],_=e.mold_threshold??70,I=e.navigate_to||((D=e.tap_action)==null?void 0:D.navigation_path)||null,y=e.history_chart??null,B=e.battery_low_threshold??20,x=ve(o.batteries),f=o.cameras[0]??null,P=o.cameras.slice(1),E=o.updates.filter(m=>m.state.state==="on");return{areaName:e.name||(r==null?void 0:r.name)||i||"",cardIcon:e.icon||(r==null?void 0:r.icon)||"mdi:home",navPath:I,hasLights:o.lights.length>0,lightCount:p.length,offlineLights:o.lights.filter(m=>m.state.state==="unavailable").length,lightColor:d,occupied:z(o.motions)||z(o.occupancy),hasOccupancySensors:o.motions.length>0||o.occupancy.length>0,problemCount:o.problems.length,showBatteryBadge:x!=null&&x.value<=B,batteryValue:(x==null?void 0:x.value)??null,batteryIcon:x?K(x.value):null,batteryEntity:(x==null?void 0:x.entityId)??null,batteryTitle:x?`${o.batteries.length>1?`Lowest of ${o.batteries.length} — `:""}${((C=x.state.attributes)==null?void 0:C.friendly_name)??x.entityId}: ${x.value}%`:"",tempVal:c,humVal:u,tempUnit:(($=(g=o.temperatures[0])==null?void 0:g.state.attributes)==null?void 0:$.unit_of_measurement)??"°C",tempEntities:o.temperatures,humEntities:o.humidities,climate:h,climIcon:v,climColor:k,smokeOn:z(o.smokes),gasOn:z(o.gases),waterOn:z(o.moistures),moldRisk:u!==null&&u>=_,updateCount:E.length,updateEntity:((ee=E[0])==null?void 0:ee.entityId)??null,updateTitle:E.length?`${E.length} update${E.length!==1?"s":""} available: ${E.map(m=>{var b;return((b=m.state.attributes)==null?void 0:b.friendly_name)??m.entityId}).join(", ")}`:"",hasCamera:e.show_camera!==!1&&!!f,cameraEntity:(f==null?void 0:f.entityId)??null,cameraImage:((te=f==null?void 0:f.state.attributes)==null?void 0:te.entity_picture)??null,cameraIcon:f?T(f.entityId,f.state):null,cameraTitle:((ae=f==null?void 0:f.state.attributes)==null?void 0:ae.friendly_name)??(f==null?void 0:f.entityId)??"",cameraState:(f==null?void 0:f.state.state)??"",cameraOffline:(f==null?void 0:f.state.state)==="unavailable",controlItems:e.show_entities!==!1?o.controls.map(({entityId:m,state:b})=>{var w;return{entityId:m,domain:m.split(".")[0],isActive:q.has(b.state),icon:T(m,b),label:J(m,b),title:`${((w=b.attributes)==null?void 0:w.friendly_name)??m} — ${b.state}`}}):[],collapsibleControls:e.collapsible_controls!==!1,controlsCollapsed:e.collapsible_controls!==!1&&n,ptzItems:e.show_entities!==!1?o.ptz.map(({entityId:m,state:b,direction:w})=>{var O;return{entityId:m,direction:w,icon:he[w],title:((O=b.attributes)==null?void 0:O.friendly_name)??m}}):[],weatherItems:e.show_entities!==!1?o.weathers.map(({entityId:m,state:b})=>{var ne,se,le;const w=parseFloat(b.state),O=((ne=b.attributes)==null?void 0:ne.unit_of_measurement)??"",Be=((se=b.attributes)==null?void 0:se.device_class)??"";return{entityId:m,dc:Be,icon:T(m,b),value:isNaN(w)?b.state:w.toFixed(1),unit:O,title:`${((le=b.attributes)==null?void 0:le.friendly_name)??m} — ${b.state}${O}`}}):[],historyPoints:y!=null&&y.entity_id?a:null,historyColor:(y==null?void 0:y.color)??"rgba(3, 169, 244, 0.12)",historyChart:y,historyMin:y!=null&&y.entity_id&&(a==null?void 0:a.length)>=2?Math.min(...a):null,historyMax:y!=null&&y.entity_id&&(a==null?void 0:a.length)>=2?Math.max(...a):null,historyUnit:((oe=(re=(ie=t.states)==null?void 0:ie[y==null?void 0:y.entity_id])==null?void 0:re.attributes)==null?void 0:oe.unit_of_measurement)??"",historyHours:(y==null?void 0:y.hours)??24,chipItems:e.show_entities!==!1?[...o.others,...P].slice(0,e.max_entities??6).map(({entityId:m,state:b})=>{var w;return{entityId:m,isActive:q.has(b.state),icon:T(m,b),label:J(m,b),title:`${((w=b.attributes)==null?void 0:w.friendly_name)??m} — ${b.state}`}}):[]}}function Ee({areaName:t,cardIcon:e,hasLights:a,lightCount:n,offlineLights:i,occupied:r,hasOccupancySensors:l,problemCount:s,showBatteryBadge:o,batteryValue:p,batteryIcon:d,batteryEntity:c,batteryTitle:u,updateCount:h,updateEntity:v,updateTitle:k}){const _=n===0,I=_?i>0?`${i} light${i!==1?"s":""} offline`:"Lights off":`${n} light${n!==1?"s":""} on${i>0?` · ${i} offline`:""}`;return`
    <div class="header">
    ...
    </div>`:""}function Ne({controlItems:t,ptzItems:e,collapsibleControls:a,controlsCollapsed:n}){return!t.length&&!e.length?"":`
    ...
    </div>`}function Oe({smokeOn:t,gasOn:e,waterOn:a,moldRisk:n}){return!t&&!e&&!a&&!n?"":`
    <div class="alarm-bar">
    ...
    </ha-card>`}function Le({historyMin:t,historyMax:e,historyUnit:a,historyHours:n,historyChart:i}){if(t===null)return"";const r=[];if((i==null?void 0:i.threshold_high)!=null||(i==null?void 0:i.threshold_low)!=null){const{min:l,range:s}=Q(i,t,e),o=s||1,p=d=>(1-(d-l)/o)*100;if(i.threshold_high!=null){const d=p(i.threshold_high);d>0&&d<100&&r.push(`<span class="chart-threshold" style="top:${d.toFixed(1)}%">${i.threshold_high.toFixed(1)}${a}</span>`)}if(i.threshold_low!=null){const d=p(i.threshold_low);d>0&&d<100&&r.push(`<span class="chart-threshold" style="top:${d.toFixed(1)}%">${i.threshold_low.toFixed(1)}${a}</span>`)}}return`
    ...
    >
      ${t.historyPoints?Se(t.historyPoints,t.historyColor,t.historyChart,t.historyUnit):""}
      ${Le(t)}
