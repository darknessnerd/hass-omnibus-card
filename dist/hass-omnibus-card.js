...
`;function I(t){const e=t.map(a=>parseFloat(a.state.state)).filter(a=>!isNaN(a));return e.length?e.reduce((a,r)=>a+r,0)/e.length:null}function _(t){return t.some(e=>e.state.state==="on")}function Z(t){return t.filter(e=>e.state.state==="on")}function J(t){var e;for(const a of t){const r=(e=a.state.attributes)==null?void 0:e.rgb_color;if(r)return`rgb(${r.join(",")})`}return null}function K(t,e){var r;return(((r=e.attributes)==null?void 0:r.friendly_name)??t.split(".")[1]).split(" ").pop()}function P(t,e){var s,i;if((s=e.attributes)!=null&&s.icon)return e.attributes.icon;const a=t.split(".")[0],r=((i=e.attributes)==null?void 0:i.device_class)??"",o=E.has(e.state),l=n=>typeof n=="string"?n:o?n.on:n.off;return r&&M[r]?l(M[r]):H[a]?l(H[a]):"mdi:help-circle-outline"}function F(t,e){t.dispatchEvent(new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:e}}))}function Q(t){history.pushState(null,"",t),window.dispatchEvent(new CustomEvent("location-changed",{bubbles:!0,composed:!0,detail:{replace:!1}}))}function X(t,e,a=null){if(!(t!=null&&t.length)||t.length<2)return"";const r=300,o=60,l=Math.min(...t),i=Math.max(...t)-l||1,n=t.map((d,x)=>x/(t.length-1)*r),c=t.map(d=>o-(d-l)/i*o),u=`${n.map((d,x)=>`${x?"L":"M"}${d.toFixed(1)},${c[x].toFixed(1)}`).join(" ")} V${o} H0 Z`;if(!(a&&(a.threshold_high!=null||a.threshold_low!=null)))return L(r,o,`<path d="${u}" fill="${e}"/>`);const y=a.color??"rgba(3, 169, 244, 0.12)",g=a.color_high??"rgba(244, 67, 54, 0.25)",b=a.color_low??"rgba(33, 150, 243, 0.25)",C=d=>Math.max(0,Math.min(o,o-(d-l)/i*o)),p=`<defs><clipPath id="sg-cp"><path d="${u}"/></clipPath></defs>`;let m=`<path d="${u}" fill="${y}"/>`;if(a.threshold_high!=null){const d=C(a.threshold_high);d>0&&(m+=`<rect x="0" y="0" width="${r}" height="${d.toFixed(1)}" fill="${g}" clip-path="url(#sg-cp)"/>`),d>0&&d<o&&(m+=`<line x1="0" y1="${d.toFixed(1)}" x2="${r}" y2="${d.toFixed(1)}" stroke="${g}" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.6"/>`)}if(a.threshold_low!=null){const d=C(a.threshold_low);d<o&&(m+=`<rect x="0" y="${d.toFixed(1)}" width="${r}" height="${(o-d).toFixed(1)}" fill="${b}" clip-path="url(#sg-cp)"/>`),d>0&&d<o&&(m+=`<line x1="0" y1="${d.toFixed(1)}" x2="${r}" y2="${d.toFixed(1)}" stroke="${b}" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.6"/>`)}return L(r,o,p+m)}function L(t,e,a){return`<svg class="bg-chart" viewBox="0 0 ${t} ${e}" preserveAspectRatio="none" aria-hidden="true">${a}</svg>`}function ee(t,e,a=null){var m,d,x,N,T,D,B,j,Y,q;const r=e.area,o=(m=t.areas)==null?void 0:m[r];if(!o&&!e.name&&!((d=e.entities)!=null&&d.length))return{error:r??"(no area)"};const l=(x=e.entities)!=null&&x.length?[]:O(t,r),s=G(l,e,t),i=V(s),n=Z(i.lights),c=J(n),h=I(i.temperatures),u=I(i.humidities),f=i.climate[0]??null,[y,g]=W[(N=f==null?void 0:f.state)==null?void 0:N.state]??[null,null],b=e.mold_threshold??70,C=e.navigate_to||((T=e.tap_action)==null?void 0:T.navigation_path)||null,p=e.history_chart??null;return{areaName:e.name||(o==null?void 0:o.name)||r||"",cardIcon:e.icon||(o==null?void 0:o.icon)||"mdi:home",navPath:C,hasLights:i.lights.length>0,lightCount:n.length,offlineLights:i.lights.filter(v=>v.state.state==="unavailable").length,lightColor:c,occupied:_(i.motions)||_(i.occupancy),hasOccupancySensors:i.motions.length>0||i.occupancy.length>0,problemCount:i.problems.length,tempVal:h,humVal:u,tempUnit:((B=(D=i.temperatures[0])==null?void 0:D.state.attributes)==null?void 0:B.unit_of_measurement)??"°C",tempEntities:i.temperatures,humEntities:i.humidities,climate:f,climIcon:y,climColor:g,smokeOn:_(i.smokes),gasOn:_(i.gases),waterOn:_(i.moistures),moldRisk:u!==null&&u>=b,historyPoints:p!=null&&p.entity_id?a:null,historyColor:(p==null?void 0:p.color)??"rgba(3, 169, 244, 0.12)",historyChart:p,historyMin:p!=null&&p.entity_id&&(a==null?void 0:a.length)>=2?Math.min(...a):null,historyMax:p!=null&&p.entity_id&&(a==null?void 0:a.length)>=2?Math.max(...a):null,historyUnit:((q=(Y=(j=t.states)==null?void 0:j[p==null?void 0:p.entity_id])==null?void 0:Y.attributes)==null?void 0:q.unit_of_measurement)??"",historyHours:(p==null?void 0:p.hours)??24,chipItems:e.show_entities!==!1?i.others.slice(0,e.max_entities??6).map(({entityId:v,state:w})=>{var R;return{entityId:v,isActive:E.has(w.state),icon:P(v,w),label:K(v,w),title:`${((R=w.attributes)==null?void 0:R.friendly_name)??v} — ${w.state}`}}):[]}}function te({areaName:t,cardIcon:e,hasLights:a,lightCount:r,offlineLights:o,occupied:l,hasOccupancySensors:s,problemCount:i}){const n=r===0,c=n?o>0?`${o} light${o!==1?"s":""} offline`:"Lights off":`${r} light${r!==1?"s":""} on${o>0?` · ${o} offline`:""}`;return`
    ...
    </div>`}function se(t){const e=t.smokeOn||t.gasOn||t.waterOn,a=t.lightColor?`background: linear-gradient(135deg, ${t.lightColor}1a 0%, var(--ha-card-background, var(--card-background-color, transparent)) 60%);`:"",r=[t.navPath?"clickable":"",e?"alarm-active":""].filter(Boolean).join(" ");return`
    <style>${z}</style>
    <ha-card
      ${r?`class="${r}"`:""}
      style="${a}"
